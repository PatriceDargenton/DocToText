...
namespace DocToText
{
    ...
    public static class EnumExtensions
    {
            ...
            where TEnum : struct, Enum
        {
            ...
            descr = AppHelpers.RemoveAfterParenthesis(descr) ?? "";
            return descr;
        } 
        */
        
        public static string ToRegistryKey<TEnum>(this TEnum enumValue)
            where TEnum : struct, Enum
        {
            var a = EnumsNET.Enums.GetMember(enumValue);
            if (a is null) return "";
            return a.Attributes.Get<RegistryKeyAttribute>()?.Key ?? "";
        }
